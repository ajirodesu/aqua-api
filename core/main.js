...
app.use('/', express.static(path.join(__dirname, 'docs')));   // docs is inside core/
...
async function loadEndpointsFromDirectory(directory, categoryPath = '') {
  let endpoints = [];
  const fullPath = path.join(rootDir, directory);   // ← apis/ is in home directory (root)
...
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'docs', 'gate.html'));
});
...
const allEndpoints = await loadEndpointsFromDirectory('apis');   // ← now correctly scans root/apis/
logger.ready(
...
app.get('/set', (req, res) => {
  ...
  res.json({ status: true, ...config, notification: notifications });
});
