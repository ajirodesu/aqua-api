...
class Greeting {
  ...
  async toAttachment() {
    ...
    if (this.assent) {
      try {
        const overlay = await loadImageOrig(this.assent);
        ctx.drawImage(overlay, 0, 0, canvas.width, canvas.height);
      } catch (e) { /* ignore if overlay fails */ }
    }
    ...
    ctx.closePath();
    ctx.clip();
    const avatarImg = await loadImageOrig(this.avatar);
    ...
    ctx.closePath();
    ctx.clip();
    const guildImg = await loadImageOrig(this.guildIcon);
...
export const meta = {
  ...
  category: 'canvas',
  params: [
    { name: 'type', desc: 'The type of greeting: welcome or goodbye', example: 'welcome', required: true },
    ...
    { name: 'guildIcon', desc: 'URL to the guild icon image', example: 'https://raw.githubusercontent.com/lanceajiro/Storage/refs/heads/main/1760059385426.jpg', required: true },
    { name: 'bg', desc: 'URL to the background image', example: 'https://raw.githubusercontent.com/lanceajiro/Storage/refs/heads/main/backiee-265579-landscape.jpg', required: true },
    { name: 'username', desc: 'The username to display', example: 'AjiroDesu', required: true },
    { name: 'guildName', desc: 'The guild name to display', example: 'My Server', required: true },
    { name: 'memberCount', desc: 'The member count to display', example: '100', required: true }
  ]
};
