...
export const meta = {
  ...
  category: 'canvas',
  params: [
    { name: 'image', desc: 'URL of the image to overlay', example: 'https://raw.githubusercontent.com/lanceajiro/Storage/refs/heads/main/1756728735205.jpg', required: true },
    { name: 'title', desc: 'Text to display as the title', example: 'Kold Bantutan', required: true }
  ]
};
...
export async function onStart({ req, res }) {
  ...
  if (!image || !title) {
    return res.status(400).json({ error: 'Missing required parameters: image and title' });
  }
  try {
