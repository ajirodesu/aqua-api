...
export const meta = {
  ...
  category: 'canvas',
  params: [
    { name: 'headline', desc: 'The main headline text for the news', example: 'Breaking: Aliens Invade Earth', required: true },
    { name: 'bg', desc: 'Optional URL to the background image (defaults to a professional news studio)', example: 'https://t3.ftcdn.net/jpg/05/10/02/94/360_F_510029428_jEAIzygxz7TDH8szCuPnkwLnENTPVPTG.jpg', required: false },
    { name: 'ticker', desc: 'Optional text for the bottom news ticker (defaults to headline)', example: 'Stay tuned for more updates...', required: false }
  ]
};
...
export async function onStart({ req, res }) {
  ...
  try {
    ...
    if (typeof ctx.roundRect === 'function') {
      ...
      ctx.arcTo(bannerX + bannerWidth, bannerY, bannerX + bannerWidth, bannerY + bannerHeight, radius);
      ctx.arcTo(bannerX + bannerWidth, bannerY + bannerHeight, bannerX, bannerY + bannerHeight, radius);
      ctx.arcTo(bannerX, bannerY + bannerHeight, bannerX, bannerY, radius);
      ctx.arcTo(bannerX, bannerY, bannerX + bannerWidth, bannerY, radius);
    }
    ctx.fill();
    ...
    if (typeof ctx.roundRect === 'function') {
      ...
      ctx.beginPath();
      const r = 10;
      ctx.moveTo(liveBoxX + r, liveBoxY);
      ctx.arcTo(liveBoxX + liveBoxWidth, liveBoxY, liveBoxX + liveBoxWidth, liveBoxY + liveBoxHeight, r);
      ctx.arcTo(liveBoxX + liveBoxWidth, liveBoxY + liveBoxHeight, liveBoxX, liveBoxY + liveBoxHeight, r);
      ctx.arcTo(liveBoxX, liveBoxY + liveBoxHeight, liveBoxX, liveBoxY, r);
      ctx.arcTo(liveBoxX, liveBoxY, liveBoxX + liveBoxWidth, liveBoxY, r);
      ctx.fill();
    }
    ctx.fillStyle = 'red';
    ctx.beginPath();
