...
// ICONS and helper functions (unchanged)
const ICONS = {
...
export async function onStart({ req, res }) {
  ...
  if (config.hasComment) {
    ...
    const bubbleHeight = 16 + 13 + 4 + textBlockH;
    const metaHeight = 16;

    commentTotalHeight = bubbleHeight + metaHeight + 4;
  }

  const textTopY = headerHeight + 4;
  const imageTopY = textTopY + textBlockHeight + 12;
  const statsTopY = imageTopY + imageHeight + 10;
  ...
  if (config.hasComment) {
    ...
    if (cAvatarImg) {
        ...
        ctx.fillRect(padding, commentY, commentAvatarSize, commentAvatarSize);
    }
    ctx.restore();
  ...
  res.send(Buffer.from(canvas.toBuffer('image/png')));
}